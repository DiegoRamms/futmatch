...
class StripeWebhookService(
    ...
    private suspend fun handlePaymentFailed(paymentIntent: PaymentIntent) {
        ...
        paymentIntent.metadata["matchId"]?.let { matchIdStr ->
            ...
            if (matchId == null) {
                logger.warn("⚠️ Invalid matchId in payment metadata for failed payment. paymentIntentId={}", paymentIntentId)
                return@let
            }
    ...
    private suspend fun handlePaymentCanceled(paymentIntent: PaymentIntent) {
        ...
        val isRequestedByCustomer = cancellationReason == "requested_by_customer"
        ...
        paymentIntent.metadata["matchId"]?.let { matchIdStr ->
            ...
            if (!isRequestedByCustomer) {
                ...
                if (userId != null) {
                    ...
                    logger.info("🔔 Payment failed notification sent for non-voluntary cancellation. userId={}, matchId={}", userId, matchId)
                } else {
                    logger.warn("⚠️ userId missing/invalid in payment metadata for non-voluntary cancellation. paymentIntentId={}", paymentIntentId)
                }
            }
