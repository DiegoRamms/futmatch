...
import com.devapplab.model.auth.response.GoogleAuthFlow
import com.devapplab.model.auth.response.GoogleAuthResolveResponse
...
class GoogleAuthService(
    ...
    suspend fun resolve(
        request: GoogleAuthResolveRequest,
        jwtConfig: JWTConfig,
        locale: Locale,
        deviceInfo: String?,
        context: AuthRequestContext
    ): AppResult<GoogleAuthResolveResponse> {
        val startedAt = System.currentTimeMillis()
        val verification = withContext(Dispatchers.IO) {
            googleIdTokenVerifier.verify(request.idToken.trim())
        }
        val verifiedIdentity = (verification as? GoogleIdTokenVerificationResult.Valid)?.identity
            ?: run {
                val reason = (verification as GoogleIdTokenVerificationResult.Invalid).reason
                logger.authEvent(
                    severity = AuthLogSeverity.WARN,
                    event = "auth.google.resolve.failed",
                    context = context,
                    outcome = "rejected",
                    reason = reason,
                ...
                return locale.createError(
                    ...
                    errorCode = ErrorCode.AUTH_GOOGLE_TOKEN_INVALID
                )
            }
        ...
        val lookup = runCatching {
            dbExecutor.tx {
                ...
                if (user.status != UserStatus.ACTIVE) {
                    return@tx GoogleResolveLookup.InactiveUser(user.status, user.userId)
                }

                if (deviceInfo.isNullOrBlank()) {
                    return@tx GoogleResolveLookup.MissingDeviceInfo
                }
                ...
                GoogleResolveLookup.Authenticated(
                    userId = user.userId,
        ...
        return when (lookup) {
            GoogleResolveLookup.SignUpRequired -> {
                logger.authEvent(
                    severity = AuthLogSeverity.INFO,
                    event = "auth.google.resolve.sign_up_required",
                    context = context,
                    outcome = "success",
                    durationMs = System.currentTimeMillis() - startedAt
                )
            ...
            GoogleResolveLookup.MissingDeviceInfo -> locale.createError(
                StringResourcesKey.AUTH_DEVICE_INFO_REQUIRED_TITLE,
                StringResourcesKey.AUTH_DEVICE_INFO_REQUIRED_DESCRIPTION,
                status = HttpStatusCode.BadRequest
            )

            is GoogleResolveLookup.InactiveUser -> resolveInactiveUser(lookup, locale, context, startedAt)

            is GoogleResolveLookup.Authenticated -> {
                ...
                if (response is AppResult.Success) {
                    logger.authEvent(
                        severity = AuthLogSeverity.INFO,
                        event = "auth.google.resolve.success",
                        context = context,
                        outcome = "success",
                        userId = lookup.userId,
                        deviceId = lookup.deviceId,
                    ...
                    AppResult.Success(
                        GoogleAuthResolveResponse(
                            flow = GoogleAuthFlow.AUTHENTICATED,
    ...
    private fun resolveInactiveUser(
        lookup: GoogleResolveLookup.InactiveUser,
        locale: Locale,
        context: AuthRequestContext,
        startedAt: Long
    ): AppResult.Failure {
    ...
    private sealed interface GoogleResolveLookup {
        ...
        data class Authenticated(val userId: UUID, val userRole: UserRole, val deviceId: UUID) : GoogleResolveLookup
    }
